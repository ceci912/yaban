...
export async function PATCH(request: Request) {
  try {
    ...
    const course = await getD1()
      .prepare("SELECT child_id, units_per_session, total_units FROM courses WHERE id = ?")
      .bind(courseId)
      .first<{ child_id: string; units_per_session: number; total_units: number }>();
    if (!course) return NextResponse.json({ error: "没有找到这门课程" }, { status: 404 });
    const session = await getD1()
      .prepare("SELECT id FROM course_sessions WHERE id = ? AND course_id = ?")
      .bind(body.sessionId, courseId)
      .first<{ id: string }>();
    if (!session) return NextResponse.json({ error: "没有找到这次课" }, { status: 404 });

    let consumedUnits = 0;
    if (body.status === "completed") {
      const usage = await getD1()
        .prepare(`
          SELECT COALESCE(SUM(consumed_units), 0) AS used_units
          FROM course_sessions
          WHERE course_id = ? AND id != ?
        `)
        .bind(courseId, body.sessionId)
        .first<{ used_units: number }>();
      consumedUnits = Math.min(
        course.units_per_session,
        Math.max(0, course.total_units - Number(usage?.used_units ?? 0)),
      );
    }
    await getD1()
      .prepare(`
